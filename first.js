...
form.addEventListener("submit", (e) => {
  ...
  //Alternative way of formData()
  let firstName = document.getElementById("name").value;
  let lastName = document.getElementById("surname").value;
  let day = Number(document.getElementById("day").value); //Coverting string to int
  let month = Number(document.getElementById("month").value); //Coverting string to int
  let year = Number(document.getElementById("year").value); //Coverting string to int

  // console.log(firstName, lastName, day, month, year);

  let first_message = `Hello ${firstName} ${lastName}.`;
  let second_message = `Your zodiac sign is ${zodiacSigns[month - 1]}`;
  let third_message = compliments[day - 1];
  let fourth_message = victimCardCompliments[Math.floor(Math.random() * 20)];

  //Ensuring to provide the user a fixed recommendation each time

  let firstNameLen = firstName.length;
  let lastNameLen = lastName.length;
  let index = (firstNameLen * lastNameLen * year) % 30; //Ensuring the index to be 0-29
  let fifth_message = recommendations[index];
  index = (day * month * year) % 20;
  let sixth_message = predictions[index];

  let res = document.getElementById("result");
  res.innerText = `${first_message} ${second_message} ${third_message} ${fourth_message} Our Reccomendation for you: ${fifth_message} Your Future Prediction is: ${sixth_message}`;
});
